...
function TransitionFunctions.transfer_planet_ownership_if_owned_by_faction(planets, required_owner_name, new_owner_name)
    return callable {
        ...
        call = function(self)
            self.Active_Planets = StoryUtil.GetSafePlanetTable()
            StoryUtil.ShowScreenText("Planets acquired", 5)

            ---@param planet Planet
            for _, planet in pairs(self.planets) do
                StoryUtil.ShowScreenText("Checking planet ".. tostring(planet), 5)
                if self.Active_Planets[planet:get_name()] then
                    StoryUtil.ShowScreenText("Planets valid ".. tostring(planet:get_name()), 5)
