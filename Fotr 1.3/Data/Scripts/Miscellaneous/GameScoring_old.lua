require("pgcommands")
require("BoardingHandler")

require("deepcore/crossplot/GlobalValueQueue")
...
function Base_Definitions()
    ...
    crossplot:master()
    crossplot:subscribe("BOARDED_UNIT", BoardingHandler.save_unit, BoardingHandler)
...
function Game_Mode_Ending_Event(mode_name)
    ...
    elseif StringCompare(mode_name, "Space") then
        BoardingHandler:finalize_boarded_unit_list()
        crossplot:publish("TACTICAL_BOARDED_UNITS", BoardingHandler.boarded_units)
        BoardingHandler:reset()
    end
end
...
function Galactic_Unit_Destroyed_Event(object, killer)
    ...
    if object_type.Is_Hero() and not object.Is_Category("Structure") then
        crossplot:publish("GALACTIC_HERO_KILLED", object_type.Get_Name(), object.Get_Owner().Get_Faction_Name())
    end
...
function Get_Military_Efficiency(player, kill_stats, build_stats)
    ...
    elseif tdpower == 0 then
        kill_eff = tpower
	elseif tdpower > tpower then
		kill_eff = -((tdpower - tpower) / tpower)
    else
        kill_eff = (tpower - tdpower) / tpower
    end
...
function Get_Conquest_Efficiency(player)
    ...
    elseif lost == 0 then
        conq_eff = sacked
	elseif lost > sacked then
		conq_eff = -((lost - sacked) / sacked)
    else
        conq_eff = (sacked - lost) / sacked
    end
...
function Get_Game_Stat_For_Control_ID(player, control_id, for_tactical)
    ...
    elseif control_id == "IDC_TITLE_STATIC" then
        ...
        ScriptError("Unknown control id %s:%s for Get_Game_Stat_For_Control_ID", type(control_id), tostring(control_id))
    end
end
