...
function Definitions()
	...
	CustomUnitPlacement = require("HeroFreeStorePlacements")
...
function Find_Custom_Target(object)
	...
	if unit_entry then
		...
		if perception then
			target = FindTarget.Reachable_Target(PlayerObject, perception, "Friendly", "Friendly_Only", 1.0, object)
		...
		if prefers_space then
			target = FindTarget.Reachable_Target(PlayerObject, "Space_Priority_Defense_Score_Hero", "Friendly", "Friendly_Only", 1.0, object)
			if TestValid(target) then
				return target
			end
			--return Find_Space_Unit_Target(object)
		else
			target = FindTarget.Reachable_Target(PlayerObject, "Ground_Priority_Defense_Score_Hero", "Friendly", "Friendly_Only", 1.0, object)
			if TestValid(target) then
				return target
			end
