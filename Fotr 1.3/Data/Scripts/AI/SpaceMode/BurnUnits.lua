...
function MainForce_Thread()
	...
	repeat
		...
		if Is_Multiplayer_Mode() == false and (EvaluatePerception("Is_Skirmish_Mode", PlayerObject) == 1) then
		...
		if target == nil then
			target = Find_Nearest(MainForce, "IsStarbase", PlayerObject, false)
			if target == nil then
				target = Find_Nearest(MainForce, "SpaceStructure | Capital | SuperCapital | Frigate", PlayerObject, false)
				if target == nil then
					break
				end
			end
